...
def makeFigure4():
    ...
    C1 = np.loadtxt('portfolio.ref')
    C2=pandas.read_csv('RVEA_portfolio.set',delimiter=' ',header=None)
    C2.columns = ['Annual', 'Min', 'Hedge', 'Complexity']
    C2=C2[C2.Min<-115]
    np.savetxt('RVEA_portfolio_req_1.txt', C2, delimiter=',')
    C2=np.loadtxt('RVEA_portfolio_req_1.txt',delimiter=',')
    ...
    plot(formulations, formulations_1, labels, precision, cmaps, titles, 'RVEA_first_requirement.pdf')
...
def plot(formulations, formulations_1,labels, precision, cmaps, titles, filename):
    ...
    for i in range(1):
        ...
        for j in range(len(labels[i])):
            ...
            if mins[j] < 0:
                labels[i][j] = labels[i][j][1:]
        parallel_coordinate_2(ax, table_1, mins, maxs, 'Purples', shadeIndex[i], titles[i], labels[i], precision[i])
